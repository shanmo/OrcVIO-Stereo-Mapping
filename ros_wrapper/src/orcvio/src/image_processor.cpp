...
#if WITH_LOG
    SPDLOG_LOGGER_DEBUG(logger, "Feature lifetime statistics:\n");
#endif 
...
bool ImageProcessor::loadParameters() noexcept {
    ...
    // initialize these to identity 
    cam0_R_Prev2Curr = cv::Matx33f::eye();
    cam1_R_Prev2Curr = cv::Matx33f::eye();
    ...
    // for image histogram equalization 
    nh.param<bool>("flag_equalize", flag_equalize, false);

#if WITH_LOG
    // for logging 
    nh.param<std::string>("output_dir_log", output_dir_log, "./cache");
    logger = spdlog::basic_logger_mt("orcvio_image_logger", output_dir_log+"/log.txt", true);
...
bool ImageProcessor::createRosIO() noexcept {
...
bool ImageProcessor::initialize() noexcept {
    if (!loadParameters())
        return false;
...
void ImageProcessor::stereoCallback(
    ...
    if (flag_equalize) {
        // equalize histogram 
        // ref https://github.com/rpng/open_vins/issues/30
        ...
        // or as in https://github.com/PetWorm/LARVIO/blob/master/src/image_processor.cpp
        // cv::Ptr<cv::CLAHE> clahe = cv::createCLAHE(3.0, cv::Size(8, 8));
        // clahe->apply(cam0_curr_img_ptr->image, cam0_curr_img_ptr->image);
        // clahe->apply(cam1_curr_img_ptr->image, cam1_curr_img_ptr->image);
    }
    ...
    ROS_INFO("Statistics: %f", (ros::Time::now()-start_time).toSec());
#endif    
...
void ImageProcessor::createImagePyramids() noexcept {
...
void ImageProcessor::initializeFirstFrame() {
    ...
    for (int i = 0; i < inlier_markers.size(); ++i) {
        if (inlier_markers[i] == 0)
            continue;
        cam0_inliers.emplace_back(cam0_points[i]);
        cam1_inliers.emplace_back(cam1_points[i]);
        response_inliers.emplace_back(new_features[i].response);
    }
    ...
    for (int i = 0; i < cam0_inliers.size(); ++i) {
        ...
        grid_new_features[code].emplace_back(new_feature);
    }
    ...
    for (int code = 0; code < processor_config.grid_row * processor_config.grid_col; ++code) {
        ...
        for (int k = 0; k < processor_config.grid_min_feature_num && k < new_features_this_grid.size(); ++k) {
            features_this_grid.emplace_back(new_features_this_grid[k]);
...
void ImageProcessor::predictFeatureTracking(
    ...
    vector<cv::Point2f> &compensated_pts) noexcept {
    ...
    cv::Matx33f K(
        ...
        0.0, 0.0, 1.0);
    cv::Matx33f H = K * R_p_c * K.inv();  // p' = K * Rcp * K-1 * p
...
void ImageProcessor::trackFeatures() {
    ...
    integrateImuData(cam0_R_Prev2Curr, cam1_R_Prev2Curr);
    ...
    for (const auto &item : *prev_features_ptr) {
        for (const auto &prev_feature : item.second) {
            prev_ids.emplace_back(prev_feature.id);
            prev_lifetime.emplace_back(prev_feature.lifetime);
            prev_cam0_points.emplace_back(prev_feature.cam0_point);
            prev_cam1_points.emplace_back(prev_feature.cam1_point);
        }
    }
    ...
    // use integrateImuData to predict cam_0_R_c, then obtain initial positions 
    predictFeatureTracking(prev_cam0_points, cam0_R_Prev2Curr, cam0_intrinsics, curr_cam0_points);

    // use LK to track features, track_inliers = 0 for outliers 
    ...
    twoPointRansac(prev_matched_cam0_points, curr_matched_cam0_points,
                   cam0_R_Prev2Curr, cam0_intrinsics, cam0_distortion_model,
    ...
    twoPointRansac(prev_matched_cam1_points, curr_matched_cam1_points,
                   cam1_R_Prev2Curr, cam1_intrinsics, cam1_distortion_model,
    ...
    for (int i = 0; i < cam0_ransac_inliers.size(); ++i) {
        ...
        int code = row * processor_config.grid_col + col;
        (*curr_features_ptr)[code].emplace_back(FeatureMetaData());
...
#if WITH_LOG
    SPDLOG_LOGGER_DEBUG(logger, "candidates: {}; raw track: {}; stereo match: {}; ransac: {}/{}={}",
...
void ImageProcessor::stereoMatch(
    ...
    // use the distance between pixel and epipolar line to remove outliers 
...
void ImageProcessor::addNewFeatures() {
    ...
    for (const auto &feature : new_features) {
        ...
        new_feature_sieve[row * processor_config.grid_col + col].emplace_back(feature);
    }
    ...
    for (auto &item : new_feature_sieve) {
        if (item.size() > processor_config.grid_max_feature_num) {
            std::sort(item.begin(), item.end(), [](const cv::KeyPoint &pt1, const cv::KeyPoint &pt2) -> bool {
...
#if WITH_LOG
    SPDLOG_LOGGER_DEBUG(logger, "detected: {}; matched: {}; new added feature:\n", 
...
void ImageProcessor::pruneGridFeatures() noexcept {
    for (auto &item : *curr_features_ptr) {
        ...
        if (grid_features.size() <= processor_config.grid_max_feature_num)
            continue;
        std::sort(grid_features.begin(), grid_features.end(), [](const FeatureMetaData &f1, const FeatureMetaData &f2) -> bool {
...
void ImageProcessor::undistortPoints(
    ...
    const cv::Vec4d &new_intrinsics) noexcept {
...
void ImageProcessor::integrateImuData(Matx33f &cam0_R_p_c, Matx33f &cam1_R_p_c) noexcept {
    
    // Find the start and the end limit within the imu msg buffer.
    auto begin_iter = std::find_if(imu_msg_buffer.begin(), imu_msg_buffer.end(), 
        [this](const auto& imu_msg) { 
            return (imu_msg.header.stamp - cam0_prev_img_ptr->header.stamp).toSec() >= -0.01; 
            });
    auto end_iter = std::find_if(begin_iter, imu_msg_buffer.end(), 
        [this](const auto& imu_msg) { 
            return (imu_msg.header.stamp - cam0_prev_img_ptr->header.stamp).toSec() >= 0.005; 
            });

    // Compute the mean angular velocity in the IMU frame.
    Vec3f mean_ang_vel(0.0, 0.0, 0.0);
    mean_ang_vel = std::accumulate(begin_iter, end_iter, mean_ang_vel, 
            [](Vec3f sum_ang_vel, const auto& imu_msg){ 
                return sum_ang_vel + Vec3f(imu_msg.angular_velocity.x, imu_msg.angular_velocity.y, imu_msg.angular_velocity.z);
            });
    ...
    {
        // if something is wrong with the timestamps, then just use the previous 
        // cam0_R_p_c, or use identity (initial value)
        
        // cannot erase any msg from imu_msg_buffer
        return; 
    }
...
void ImageProcessor::rescalePoints(vector<Point2f> &pts1, vector<Point2f> &pts2, float &scaling_factor) noexcept {
    scaling_factor = 0.0f;
...
void ImageProcessor::publish() {
    ...
    for (const auto &grid_features : (*curr_features_ptr)) {
        for (const auto &feature : grid_features.second) {
            curr_ids.emplace_back(feature.id);
            curr_cam0_points.emplace_back(feature.cam0_point);
            curr_cam1_points.emplace_back(feature.cam1_point);
        }
    }
    ...
    for (int i = 0; i < curr_ids.size(); ++i) {
        feature_msg_ptr->features.emplace_back(FeatureMeasurement());
    ...
    // features are in normalized coordinates 
...
void ImageProcessor::drawFeaturesStereo() {
    if (debug_stereo_pub.getNumSubscribers() > 0) {
        ...
        for (const auto &grid_features : *prev_features_ptr)
            for (const auto &feature : grid_features.second)
                prev_ids.emplace_back(feature.id);
...
#if WITH_LIFETIME_STATISTICS
void ImageProcessor::updateFeatureLifetime() noexcept {
    for (int code = 0; code < processor_config.grid_row * processor_config.grid_col; ++code) {
...
void ImageProcessor::featureLifetimeStatistics() noexcept {
