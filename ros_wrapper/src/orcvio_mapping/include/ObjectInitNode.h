...
class ObjectInitNode
{
  ...
  public:
     ...
     */
    void get_times_cam_poses(const std::vector<double>& clonetimes, 
     ...
     */
    void add_gt_object_state(const std::string& object_class, const int& object_id, 
                const Eigen::MatrixXd & object_position_gt_mat, const Eigen::MatrixXd & object_rotation_gt_mat);
    ...
    double min_dist_btw_objects;
    double max_dist_btw_cam_obj;

    double calc_min_distance(ObjectState& new_object);
    std::tuple<double, double> calc_obj_cam_distance(
